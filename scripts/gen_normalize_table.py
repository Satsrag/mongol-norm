...
this build-time script, NOT in the runtime engine. The engine (the Rust crate
``crates/mongol-norm``, which the Python package wraps) compiles the table in from
the JSON this script emits (``scripts/gen_rust_tables.py``). Keeping the battery
...
JSON (``mongol_norm/data/<LOCALE>.json``, see docs/data-format.md).
...
selection battery, commit the regenerated JSON, then regenerate the Rust tables:
    ...
    python scripts/gen_normalize_table.py --check    # exit 1 if a bundled table is stale
    python scripts/gen_rust_tables.py                # JSON -> crates/mongol-norm/src/generated/
...
# directory, e.g. with `maturin develop`).
...
def main() -> int:
    ...
    if stale:
        ...
        print("regenerate with: python scripts/gen_normalize_table.py "
              "(then scripts/gen_rust_tables.py)", file=sys.stderr)
        return 1
    return 0
